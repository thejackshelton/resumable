...
Need async UI state? Use Qwik AsyncSignal.
...
The exported query is callable from Qwik UI. A query call returns the same
`AsyncSignal<T>` shape as Qwik's `createAsync$`: `.value`, `.loading`, and
`.error` are the primary UI contract.

```tsx
const post = getPost({ slug: "hello" });
...
Resumable should implement the component-side reader by composing with Qwik's
`createAsync$`/AsyncSignal primitive and seeding it from SSR or SPA navigation
query records. Users should not need to wrap a query in `createAsync$`,
`useAsync$`, or a Resumable-specific `useQuery` hook for the common path.
...
export default component$((props: PageProps) => {
  const post = getPost({ slug: props.params.slug });
...
`query$` does not replace `createAsync$`.

`query$` defines the server read contract.

`createAsync$` creates async UI state inside a Qwik component.

Calling a `query$` export in Qwik UI returns an `AsyncSignal<T>` using that same
state model, with `.value`, `.loading`, `.error`, `.promise()`,
`.invalidate()`, and `.abort()` semantics coming from Qwik.
...
- `schema` is optional.
- Calling a query export in Qwik UI returns an `AsyncSignal<T>` compatible with
  Qwik `createAsync$`.
- The query `AsyncSignal<T>` exposes `.value`, `.loading`, `.error`,
  `.promise()`, `.invalidate()`, and `.abort()` through Qwik's async signal
  model.
...
- Query output and resolved signal state are serializable by Qwik.
...
- Qwik v2 AsyncSignal and `<Suspense>` docs in the upstream Qwik repository:
